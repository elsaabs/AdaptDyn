...
stable_aat_df <- read.table('stable_c0=1.24', sep=" ", dec='.', header=F)
...
# title("eco 2100", outer=F)

# # Save Ctot
# Ctot = Csol*1000
# Ctot = as.matrix(Ctot)
# write.table(Ctot, file = "soilC_c0=1.24_eco_2100", append = FALSE, quote = TRUE, sep = " ", dec = ".", row.names = FALSE, col.names = FALSE)

length(which(!is.na(Csol)))

# Map change in soil C without evolution for c0=1.17
soilCeco2010_df <- read.table('soilC_c0=1.17_eco_2010',sep= " ",dec='.',header=F)
soilCeco2010 <- as.matrix(soilCeco2010_df)
soilCeco2100_df <- read.table('soilC_c0=1.17_eco_2100',sep= " ",dec='.',header=F)
soilCeco2100 <- as.matrix(soilCeco2100_df)

min(soilCeco2100-soilCeco2010,na.rm=T)
...
brks <- seq(-2000, 2000, by=0.1)
colfunc <- colorRampPalette(c(
  "#a50026","#d73027","#f46d43","#fdae61","#fee090","gray48",
  "#e0f3f8","#abd9e9","#74add1","#4575b4","#313695"))(length(brks)-1)
fields::image.plot(x=lon, y=lat, soilCeco2100-soilCeco2010, ann=FALSE, breaks=brks,
