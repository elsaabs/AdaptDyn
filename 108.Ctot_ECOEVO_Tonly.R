...
# fields::image.plot(x=lon, y=lat, stable2010, ann=F, breaks=brks,
#                    horizontal=T, smallplot=c(0.16, 0.92, 0.01, 0.04),
...
# lat = ncvar_get(nclitt, "lat")
# par(xpd=T, mfrow=c(1,1), lwd=1, font.lab=1, font.axis=1, oma=c(3,0,0,0))
# brks = seq(0, 0.18, by=0.01)
# colfunc = colorRampPalette(c(
#   "white","blue","green","yellow","orange","red"))(length(brks)-1)
# fields::image.plot(x=lon, y=lat, phi, ann=F, breaks=brks,
#                    horizontal=T, smallplot=c(0.16, 0.92, 0.01, 0.04),
#                    legend.args=list(text='Enzyme allocation, phi',
#                                     side=1, font=1, line=2, cex=1),
#                    col=colfunc,
#                    xlab="Longitude", ylab="Latitude")
# mtext(side=1, text="Longitude", line=2.2, font=1)
...
# length(which(stable2010==1))

# # Save stable 2010
# stable = as.matrix(stable)
# write.table(stable, file = "stable2100_ecoevo_c0=1.24", append = FALSE, quote = TRUE, sep = " ", dec = ".", row.names = FALSE, col.names = FALSE)

# # Save phiecoevo2010
# phi = as.matrix(phi)
# write.table(phi, file = "phiecoevo2010_c0=1.24", append = FALSE, quote = TRUE, sep = " ", dec = ".", row.names = FALSE, col.names = FALSE)
